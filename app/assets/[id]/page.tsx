...
export default function AssetDetailsPage() {
  ...
  useEffect(() => {
    ...
    try {
       ...
       * OBJECT-LEVEL ACCESS
       *
       * Employee can view ONLY an
       * asset currently assigned to
       * that employee.
       */
      if (
        parsedCurrentUser.role ===
      ...
      /*
       * Asset history is loaded for
       * Admin/Support.
       *
       * Employee UI does not render
       * the history section.
       */
      const currentAssetHistory =
        storedHistory
          .filter(
            (record) =>
      ...
      /*
       * Related ticket privacy:
       *
       * Admin + Support:
       * all tickets for the asset.
       *
       * Employee:
       * only their own tickets for
       * their assigned asset.
       */
      const currentRelatedTickets =
        allTickets
          .filter((ticket) => {
            ...
            const ticketEmployeeName =
              ticket.employeeName
                ?.toLowerCase()
                .trim() || "";

            const ticketEmployeeId =
              ticket.employeeId
                ?.toLowerCase()
                .trim() || "";
      ...
      /*
       * Do not expose fallback assets
       * to Employee if ownership
       * cannot be verified.
       */
      if (
        parsedCurrentUser.role ===
  ...
  function deleteAsset() {
    ...
    try {
      ...
      const updatedDeletedAssetIds =
        Array.from(
          new Set([
            ...savedDeletedAssetIds.map(
              (id) =>
                id.toUpperCase(),
            ),
            asset.id.toUpperCase(),
  ...
  /*
   * Asset tracking administration
   * is available to Admin/Support.
   *
   * Employee still sees the QR
   * representation of their own
   * asset, but not operational
   * QR/label controls.
   */
  ...
  /*
   * Assignment/status audit data
   * is operational IT information,
   * not employee self-service data.
   */
  const canViewAssetHistory =
    currentUser?.role !==
    "Employee";
      ...
      <section className="min-w-0 flex-1 p-6 md:p-10">
        <div className="mx-auto max-w-5xl">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            {/* Asset History - Admin / Support only */}
            {canViewAssetHistory && (
              <section className="rounded-2xl bg-zinc-900 p-6 md:col-span-2">
                <div className="mb-6">
